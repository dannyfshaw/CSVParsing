...
//	private static final String SAMPLE_CSV_FILE = "./output_Danny.csv";
	...
	// CSV file header
	private static final Object[] FILE_HEADER = { "FirstName", "Surname", "Employee Number", "Branch", "Product",
			"Size", "Qty" };
	...
	public void writeCsvFile(String fileName, List<OutCSVFormat> listOfOutboundCSVFormat) {
		...
		try {
			...
			for (OutCSVFormat item : listOfOutboundCSVFormat) {
				String[] splitName = item.getFullName().split(" ");
				String firstName = "";
				String surName = "";
				...
				System.out.println("GOT THIS item.getFullName::" + item.getFullName());
				String[] rowValues = { firstName, surName, item.getEmployeeNumber(), item.getBranch(),
						item.getProduct(), item.getSize(), Integer.toString(item.getQuantity()) };
	...
	// }
	//
	//// private List<OutCSVFormat> mapInboudToOutbuondFormat(List<InboundCSVFormat>
	// listOfInboundCSVFormat) {
